...
object reference mapping is a means to connect the code of a project to a database. It does this by translating the
c# code into the sql commands nessessary to make a database that matches the code through migrations
...
3. When adding a new pizza table to our database, which of the following two commands would we run first? Why is it important to run these two line in that order?
    ...
    update-database
    ```
    we need to run add-migration first because update-database runs any migrations that haven't been run yet and if you haven't added a migration than update-database does nothing.
...
Fork the Replit below, and complete the 5 LINQ exercises.  Add a link to your forked repl here: https://replit.com/@RafiWick/Mod2Week4Assessment-rafi#main.cs
